...
type
  ...
  public
    ...
    property List: TList read FList; // PPointerList
...
begin
  ...
  if FList.Count > Index then
    System.Move(FList[Index + 1],
                FList[Index],
                (FList.Count - Index) * SizeOf(Pointer));
end;}
...
begin
  ...
  {if (FList.Count > EndIndex + 1) then
  begin
    System.Move(FList[EndIndex + 1],
                FList[StartIndex],
                (FList.Count - (EndIndex - StartIndex + 1)) * SizeOf(Pointer));
  end;
  Dec(FList.Count, EndIndex - StartIndex + 1);
  If FList.Count < FList.Capacity shr 1 Then
  Begin
    FList.Capacity := FList.Capacity shr 1;
    ReAllocMem(FList, FCapacity * SizeOf(Pointer));
  End;}
end;
