...
begin
  ...
  with GlyphPos do
  begin
    Inc(X, Client.Left + _offset.X); // range fix
    Inc(Y, Client.Top + _offset.Y); // range fix
  end;
  OffsetRect(TextBounds, TextPos.X + Client.Left + _offset.X,
    TextPos.Y + Client.Top + _offset.X); // range fix
end;
...
begin
  ...
  begin
    ...
    Msg.LParam := LPARAM(Self); // Longint
    Msg.Result := 0;
