...
{$IFDEF RX_D3}
  if FormatSettings.ShortDateFormat[1] = 'g' then { skip over prefix text } // range fix
    ScanToNumber(S, Pos);
{$ENDIF RX_D3}
  if not (ScanNumber(S, MaxInt, Pos, N1) and ScanChar(S, Pos, FormatSettings.DateSeparator) and   // range fix
    ScanNumber(S, MaxInt, Pos, N2)) then Exit;
  if ScanChar(S, Pos, FormatSettings.DateSeparator) then begin  // range fix
  ...
  ScanChar(S, Pos, FormatSettings.DateSeparator); // range fix
  ScanBlanks(S, Pos);
{$IFDEF RX_D3}
  if SysLocale.FarEast and (System.Pos('ddd', FormatSettings.ShortDateFormat) <> 0) then // range fix
  begin { ignore trailing text }
    if FormatSettings.ShortTimeFormat[1] in ['0'..'9'] then  { stop at time digit }  // range fix
      ScanToNumber(S, Pos)
      ...
      until (Pos > Length(S)) or
        (AnsiCompareText(FormatSettings.TimeAMString, Copy(S, Pos, Length(FormatSettings.TimeAMString))) = 0) or // range fix
        (AnsiCompareText(FormatSettings.TimePMString, Copy(S, Pos, Length(FormatSettings.TimePMString))) = 0);   // range fix
  end;
{$ENDIF RX_D3}
...
begin
  if Length(S) > 0 then
    for Result := 1 to 12 do begin
      if (Length(FormatSettings.LongMonthNames[Result]) > 0) and  // range fix
        (AnsiCompareText(Copy(S, 1, MaxLen),
        Copy(FormatSettings.LongMonthNames[Result], 1, MaxLen)) = 0) then Exit;   // range fix
    end;
  Result := 0;
end;
...
function StrToDateDef(const S: string; Default: TDateTime): TDateTime;
begin
  if not InternalStrToDate(FormatSettings.ShortDateFormat, S, Result) then  // range fix
...
begin
  if FourDigitYear then begin
    case GetDateOrder(FormatSettings.ShortDateFormat) of   // range fix
...
{$IFDEF USE_FOUR_DIGIT_YEAR}
initialization
  FourDigitYear := Pos('YYYY', AnsiUpperCase(FormatSettings.ShortDateFormat)) > 0; // range fix
{$ENDIF}
end.