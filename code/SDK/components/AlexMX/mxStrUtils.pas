...
begin
  ...
  if Length(Result) > 0 then
    CharToOemBuff(PChar(AnsiStr), PAnsiChar(Result), Length(Result)); // range fix
end;
...
begin
  ...
  if Length(Result) > 0 then
    OemToCharBuff(PAnsiChar(OemStr), PChar(Result), Length(Result)); // range fix
end;
...
begin
  ...
  if Length(Key) > 0 then
    for I := 1 to Length(Src) do
      Result[I] := AnsiChar(Chr(Byte(Key[1 + ((I - 1) mod Length(Key))]) xor Ord(Src[I]))); // range fix
end;
