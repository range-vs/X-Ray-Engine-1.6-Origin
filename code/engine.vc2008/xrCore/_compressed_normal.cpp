...
u16 pvCompress				( const Fvector& vec )
{
	...
	if ( negative(tmp.x) ) { mVec |= pvXSIGN_MASK; tmp.x = -tmp.x; }
	if ( negative(tmp.y) ) { mVec |= pvYSIGN_MASK; tmp.y = -tmp.y; }
	if ( negative(tmp.z) ) { mVec |= pvZSIGN_MASK; tmp.z = -tmp.z; }
...
void pvDecompress		( Fvector& vec, u16 mVec )
{
	...
	if ( mVec & pvXSIGN_MASK ) vec.x = -vec.x;
	if ( mVec & pvYSIGN_MASK ) vec.y = -vec.y;
	if ( mVec & pvZSIGN_MASK ) vec.z = -vec.z;
}
