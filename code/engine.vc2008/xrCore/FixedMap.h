...
//	}
//	IC void		getLR(TNode* N, xr_vector<T, typename xr_allocator::helper<T>::result>& D)
//	{
...
private:
	...
	IC void		getLR(TNode* N, xr_vector<T, allocate>& D)
	{
		if (N->left)	getLR(N->left, D);
		D.push_back(N->val);
		if (N->right)	getLR(N->right, D);
	...
	template <class allocate>
	IC void		getRL(TNode* N, xr_vector<T, allocate>& D)
	{
		if (N->right)	getRL(N->right, D);
		D.push_back(N->val);
		if (N->left)	getRL(N->left, D);
	...
	template <class allocate>
	IC void		getLR_P(TNode* N, xr_vector<TNode*, allocate>& D)
	{
		if (N->left)	getLR_P(N->left, D);
		D.push_back(N);
		if (N->right)	getLR_P(N->right, D);
	...
	template <class allocate>
	IC void		getRL_P(TNode* N, xr_vector<TNode*, allocate>& D)
	{
		if (N->right)	getRL_P(N->right, D);
		D.push_back(N);
		if (N->left)	getRL_P(N->left, D);
...
public:
	...
	template <class allocate>
	IC void		getLR(xr_vector<T, allocate>& D)
	{
		if (pool)	getLR(nodes, D);
	}

	template <class allocate>
	IC void		getLR_P(xr_vector<TNode*, allocate>& D)
	{
		if (pool)	getLR_P(nodes, D);
	}

	template <class allocate>
	IC void		getRL(xr_vector<T, allocate>& D)
	{
		if (pool)	getRL(nodes, D);
	}
	template <class allocate>
	IC void		getRL_P(xr_vector<TNode*, allocate>& D)
	{
		if (pool)	getRL_P(nodes, D);
	}
	template <class allocate>
	IC void		getANY(xr_vector<T, allocate>& D)
	{
		TNode* _end = end();
		for (TNode* cur = begin(); cur != _end; cur++) D.push_back(cur->val);
	}
	template <class allocate>
	IC void		getANY_P(xr_vector<TNode*, allocate>& D)
	{
		D.resize(size());
		TNode** _it = &*D.begin();
		TNode* _end = end();
	...
	template <class allocate>
	IC void		getANY_P(xr_vector<void*, allocate>& D)
	{
		D.resize(size());
		void** _it = &*D.begin();
		TNode* _end = end();
