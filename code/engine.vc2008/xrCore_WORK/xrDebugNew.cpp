	...
	if (!IsDebuggerPresent() && !strstr(GetCommandLine(),"-no_call_stack_assert")) {
		...
		for (int i=2; i<g_stackTraceCount; ++i) {
			if (shared_str_initialized)
				...
				#ifdef USE_OWN_ERROR_MESSAGE_WINDOW
			buffer += xr_sprintf(buffer, assertion_size - u32(buffer - buffer_base), "%s%s", g_stackTraceCount[i], endline);
#endif // USE_OWN_ERROR_MESSAGE_WINDOW
		}*/
		...
		for (int i = 2; i < stackTrace.size(); ++i)
		{
			Msg("%s", stackTrace[i].c_str());
#ifdef USE_OWN_ERROR_MESSAGE_WINDOW
			buffer += xr_sprintf(buffer, assertion_size - u32(buffer - buffer_base), "%s%s", stackTrace[i].c_str(), endline);
#endif // USE_OWN_ERROR_MESSAGE_WINDOW
		}
...
#		endif // USE_BUG_TRAP*/
//fdef _DEBUG
				LogStackTrace("Stack trace:"); // вывод в log stack trace
//endif
				DEBUG_INVOKE;
				break;
			}
...
int out_of_memory_handler	(size_t size)
{
	if ( g_full_memory_stats_callback )
		...
		Memory.mem_compact	();
#ifndef _EDITOR
		u32					crt_heap		=  0; //mem_usage_impl((HANDLE)_get_heap_handle(),0,0);
#else // _EDITOR
		u32					crt_heap		= 0;
#endif // _EDITOR
