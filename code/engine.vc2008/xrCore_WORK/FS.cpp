...
#ifdef _EDITOR
bool file_handle_internal	(LPCSTR file_name, u32 &size, s32 &hFile)
{
...
#else // EDITOR
static errno_t open_internal(LPCSTR fn, s32 &handle)
{
	return				(
		_sopen_s(
			&handle,
...
bool file_handle_internal	(LPCSTR file_name, u32 &size, s32 &file_handle)
{
...
void *FileDownload		(LPCSTR file_name, const s32 &file_handle, u32 &file_size)
{
	...
	s32					r_bytes	= _read(file_handle,buffer,file_size);
	R_ASSERT3			(
//		!file_size ||
...
#include <sstream>
template<typename T>
	std::string _to_string(const T &n)
	{
        std::ostringstream s;
        s << n;
        return s.str();
