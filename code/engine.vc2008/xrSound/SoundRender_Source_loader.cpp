...
{ 
	...
	//size_t exist_block	= _max(0ul,iFloor(F->elapsed()/(float)size));
        size_t exist_block	= _max((double)0,floor((double)F->elapsed()/(double)size));
...
void CSoundRender_Source::LoadWave	(LPCSTR pName)
{
	...
	OggVorbis_File			ovf;
	ov_callbacks ovc		= {ov_read_func,ov_seek_func,/*ov_close_func*/NULL,ov_tell_func};
	IReader* wave			= FS.r_open		(pname.c_str());
	R_ASSERT3				(wave&&wave->length(),"Can't open wave file:",pname.c_str());
        int result = ov_open_callbacks		(wave,&ovf,NULL,0,ovc);
