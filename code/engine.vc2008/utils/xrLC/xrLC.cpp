...
{
	args = lpCmdLine; // присваиваем раздел€емой переменной аргументы команднйо строки
	DWORD mainThreadId; // статус создани€ потока
	mainThread = CreateThread(NULL, 0, Startup, NULL, 0, &mainThreadId); // создаем поток - и присваиваем хэндл глобальной переменной
	if (mainThread == NULL) // поток не создалс€
...
//	g_dummy_stuff		= &trivial_encryptor::encode;
	...
	CreateMainThread(lpCmdLine); // создаЄм и запускаем поток
	WaitForSingleObject(mainThread, INFINITE); // здесь ждем завершени€ вышесозданного потока
