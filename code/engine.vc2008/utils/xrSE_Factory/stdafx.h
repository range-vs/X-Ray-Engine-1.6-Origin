...
#	define	THROW(expr)				do {if (!(expr)) {std::string assertion_info = ::Debug.gather_info(_TRE(#expr),   0,   0,0,DEBUG_INFO); throw assertion_info.c_str();}} while(0)
#	define	THROW2(expr,msg0)		do {if (!(expr)) {std::string assertion_info = ::Debug.gather_info(_TRE(#expr),msg0,   0,0,DEBUG_INFO); throw assertion_info.c_str();}} while(0)
#	define	THROW3(expr,msg0,msg1)	do {if (!(expr)) {std::string assertion_info = ::Debug.gather_info(_TRE(#expr),msg0,msg1,0,DEBUG_INFO); throw assertion_info.c_str();}} while(0)
