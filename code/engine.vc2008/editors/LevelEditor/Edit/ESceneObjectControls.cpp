...
bool TUI_ControlObjectAdd::Start(TShiftState Shift)
{
    ...
    { // pick already executed (see top)
        ...
        if (ot->IsAppendRandomActive()&&ot->m_AppendRandomObjects.size()){
        	N = ot->m_AppendRandomObjects[rand.randI(ot->m_AppendRandomObjects.size())].c_str();
        }else{
...
*/
        if (ot->IsAppendRandomActive()){
            ...
            if (ot->IsAppendRandomRotationActive()){
            	Fvector p;
				p.set(	rand.randF(ot->m_AppendRandomMinRotation.x,ot->m_AppendRandomMaxRotation.x),
						rand.randF(ot->m_AppendRandomMinRotation.y,ot->m_AppendRandomMaxRotation.y),
						rand.randF(ot->m_AppendRandomMinRotation.z,ot->m_AppendRandomMaxRotation.z));
                obj->PRotation = p;
            }
            if (ot->IsAppendRandomScaleActive()){
            	Fvector s;
                if (ot->IsAppendRandomScaleProportional()){
					s.x		= rand.randF(ot->m_AppendRandomMinScale.x,ot->m_AppendRandomMaxScale.x);
                    s.set	(s.x,s.x,s.x);
                }else{
					s.set(	rand.randF(ot->m_AppendRandomMinScale.x,ot->m_AppendRandomMaxScale.x),
							rand.randF(ot->m_AppendRandomMinScale.y,ot->m_AppendRandomMaxScale.y),
							rand.randF(ot->m_AppendRandomMinScale.z,ot->m_AppendRandomMaxScale.z));
                }
                obj->PScale = s;
            }
        }
