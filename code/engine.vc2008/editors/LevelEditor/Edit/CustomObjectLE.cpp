...
void CCustomObject::FillProp(LPCSTR pref, PropItemVec& items)
{
	...
	V = PHelper().CreateVector	(items, PrepareKey(pref,"Transform\\Position"),	&GetPositionRW(),	-1000000,	1000000,0.001,3); // fix coords
    V->OnChangeEvent.bind		(this,&CCustomObject::OnNumChangePosition);
	V = PHelper().CreateAngle3	(items, PrepareKey(pref,"Transform\\Rotation"),	&GetRotationRW(),	-10000,	10000,0.01,2); // fix rotates
    V->OnChangeEvent.bind		(this,&CCustomObject::OnNumChangeRotation);
	V = PHelper().CreateVector	(items, PrepareKey(pref,"Transform\\Scale"),	&GetScaleRW(), 	0.01,	10000,0.01,2);     // fix scales
