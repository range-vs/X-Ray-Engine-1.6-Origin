...
void CPortal::Render(int priority, bool strictB2F)
{
	if ((1==priority)&&(false==strictB2F)){
		...
		if (m_SectorFront){
			...
			DU_impl.DrawPrimitiveL	(D3DPT_TRIANGLEFAN, V.size()-2, &*V.begin(), V.size(), col.get(), true, false); // range fixc
			EDevice.SetRS(D3DRS_CULLMODE,D3DCULL_CCW);
        }
        // back
		if (m_SectorBack){
		...
		DU_impl.DrawPrimitiveL	(D3DPT_LINESTRIP, src_ln.size(), &*src_ln.begin(), src_ln.size(), col.get(), true, true); // range fixc
		EDevice.ResetNearer	();
...
bool CPortal::FrustumPick(const CFrustum& frustum){
	if (frustum.testPolyInside(&*m_Vertices.begin(),m_Vertices.size())) return true; // range fixc
    return false;
}
//------------------------------------------------------------------------------
...
void CPortal::Simplify()
{
	...
	Mgc::ConvexHull2D Hull(points.size(),(const Mgc::Vector2*)&*points.begin());  // range fixc
