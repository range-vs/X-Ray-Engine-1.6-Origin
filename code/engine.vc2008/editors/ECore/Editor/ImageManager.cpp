...
bool Surface_Load(LPCSTR full_name, U32Vec& data, u32& w, u32& h, u32& a)
{
	...
	if (ext==".tga"){
        ...
        if (bm){
			...
			data.resize		(w*h);
			for (int y=h-1; y>=0; y--)
				 CopyMemory(&*data.begin()+(h-y-1)*w,FreeImage_GetScanLine(bm,y),w4); // range fix
...
void CImageManager::MakeThumbnailImage(ETextureThumbnail* THM, u32* data, u32 w, u32 h, u32 a)
{
	...
	imf_Process(&*THM->m_Pixels.begin(),THUMB_WIDTH,THUMB_HEIGHT,data,THM->_Width(),THM->_Height(),imf_box); // range fix
    THM->VFlip();
}
...
void CImageManager::CreateGameTexture(LPCSTR src_name, ETextureThumbnail* thumb)
{
	...
	MakeGameTexture(THM,game_name,&*data.begin());  // range fix
...
bool CImageManager::MakeGameTexture(ETextureThumbnail* THM, LPCSTR game_name, u32* load_data)
{
	...
	int res = DXTCompress(game_name, (u8*)load_data, (u8*)(ext_data.empty()?0:&ext_data[0]), w, h, w4, &THM->m_TexParams, 4);
	if (1!=res)
	{
		if (-1000!=res)
...
//.             MakeThumbnailImage(THM,data.begin(),w,h,a);
    	...
    	if (bForceGame||(sync_game&&bGame)){
			...
			if (IsValidSize(w,h)){
				...
				FS.update_path			(game_name,_game_textures_,game_name);
				if (MakeGameTexture(THM,game_name,&data.at(0)))// range fix
				{
...
BOOL CImageManager::Check_Compliance(LPCSTR fname, int& _compl)
{
    ...
    try {
		imf_Process     (pScaled,	w_2,h_2,&*data.begin(),w,h,imf_lanczos3	);  // range fix
...
//	EPrefs.scene_clear_color 	= 	0x00333333;
    ...
    if (EDevice.MakeScreenshot(pixels,w,h)){
		...
		tex.CreateFromData(&*pixels.begin(),w,h,obj->GetFaceCount(),obj->GetVertexCount());// range fix
        tex.Save(age);
    }else{
    	bResult = FALSE;
...
BOOL CImageManager::CreateSmallerCubeMap(LPCSTR src_name, LPCSTR dst_name)
{
    ...
    if (Surface_Load(full_name,data,wf,h,a)){
        ...
        CTimer T; T.Start();
		ETOOLS::SimplifyCubeMap	(&*data.begin(),w,h,&*sm_data.begin(),sm_w,sm_h,16.f,pb_callback,PB); // range fix
