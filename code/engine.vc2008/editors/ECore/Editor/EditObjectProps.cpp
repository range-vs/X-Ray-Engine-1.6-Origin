...
void CEditableObject::FillSurfaceProps(CSurface* SURF, LPCSTR pref, PropItemVec& items, ListItem* owner)
{
    PropValue* V;

    if(owner == NULL)
        PHelper().CreateRText(items,PrepareKey(pref,"Name"),&SURF->_NameEditable());
    else
        PHelper().CreateName(items, PrepareKey(pref,"Name"), &SURF->_NameEditable(), owner);
...
void CEditableObject::FillBasicProps(LPCSTR pref, PropItemVec& items)
{
	...
	// range fix begin
	AnsiString crTime = AnsiString(Trim(AnsiString(ctime(&m_CreateTime))));
	PHelper().CreateCaption		(items, PrepareKey(pref,"Version\\Creation Time"),crTime.c_str());
	AnsiString modTome = AnsiString(Trim(AnsiString(ctime(&m_ModifTime))));
	PHelper().CreateCaption		(items, PrepareKey(pref,"Version\\Modified Time"),modTome.c_str());
	// range fix end
