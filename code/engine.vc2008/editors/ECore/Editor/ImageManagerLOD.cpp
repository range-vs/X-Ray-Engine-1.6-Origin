...
typedef xr_vector< Fvector4 > Fvector4Vec; typedef Fvector4Vec::iterator Fvector4It;
...
typedef xr_vector< SBuildLight > BLVec; typedef BLVec::iterator BLIt;
...
void CreateLODSamples(const Fbox& bbox, U32Vec& tgt_data, u32 tgt_w, u32 tgt_h)
{
    ...
    for (int frame=0; frame<LOD_SAMPLE_COUNT; frame++){
		...
		imf_Process				(&*d_pixels.begin(),tgt_w,tgt_h,&*s_pixels.begin(),tgt_w*lod_ss_quality,tgt_h*lod_ss_quality,imf_box);  // range fix
		// copy LOD to final
		for (u32 y=0; y<tgt_h; y++)
			CopyMemory			(&*tgt_data.begin()+y*pitch+frame*tgt_w,&*d_pixels.begin()+y*tgt_w,tgt_w*sizeof(u32));// range fix
	}
...
void CImageManager::CreateLODTexture(CEditableObject* OBJECT, LPCSTR tex_name, u32 tgt_w, u32 tgt_h, int samples, int age, int quality)
{
	...
	I->Create					(tgt_w*samples,tgt_h,&*lod_pixels.begin());// range fix
//	I->Vflip					();
    I->SaveTGA					(out_name);
    ...
    strcpy						(src_name, EFS.ChangeFileExt(src_name,".tga").c_str());
    FS.update_path				(out_name,_textures_,src_name);
    I->Create					(tgt_w*samples,tgt_h,&*nm_pixels.begin()); // range fix
//	I->Vflip					();
    I->SaveTGA					(out_name);
    FS.set_file_age				(out_name,age);
    SynchronizeTexture			(src_name,age);
