...
struct CEdgePath {
	...
	class CDataStorage : public CVertexPath<bEuclidianHeuristics>::template CDataStorage<DataStorageEdgePath<_vertex>::_vertex> {// range fix template
	public:
		typedef typename CVertexPath<
			bEuclidianHeuristics
			>::template CDataStorage<
				DataStorageEdgePath<_vertex>::_vertex
			>											inherited; // range fix template
