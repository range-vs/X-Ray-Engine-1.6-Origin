...
/*IC*/	void CAI_ObjectLocation::game_vertex						(CVertex  /*const*/ *game_vertex) // range fix
{
...
/*IC*/	void CAI_ObjectLocation::game_vertex						(const _GRAPH_ID /*const*/ &game_vertex_id) // range fix
{
	VERIFY				(ai().game_graph().valid_vertex_id(game_vertex_id));
	m_game_vertex_id	= game_vertex_id;
}

/*IC*/	const CGameGraph::CVertex *CAI_ObjectLocation::game_vertex	() const // range fix
{
	VERIFY				(ai().game_graph().valid_vertex_id(m_game_vertex_id));
	return				(ai().game_graph().vertex(m_game_vertex_id));
}

/*IC*/	void CAI_ObjectLocation::level_vertex						(CLevelVertex  /*const*/ *level_vertex) // range fix
{
...
/*IC*/	void CAI_ObjectLocation::level_vertex						(const u32 /*const*/ &level_vertex_id) // range fix
{
	VERIFY				(ai().level_graph().valid_vertex_id(level_vertex_id));
	m_level_vertex_id	= level_vertex_id;
}

/*IC*/	const CLevelGraph::CVertex *CAI_ObjectLocation::level_vertex() const // range fix
{
