...
protected:
	mutable functor_type					m_functor;
...
public:
    ...
    return_type operator()(Args&&... args) const
    {
            ...
            try
            {
                if (m_functor)
                {
                    ...
                    if (m_object)
                    {
                        VERIFY(m_object.is_valid());
                        return return_type(
                            luabind::call_function<return_type>(m_functor, m_object, std::forward<Args>(args)...));
                    }
                    return return_type(luabind::call_function<return_type>(m_functor, std::forward<Args>(args)...));
                }
            }
        ...
        {
            const_cast<CScriptCallbackEx_<return_type>*>(this)->clear();
        }
...
template <>
template <typename... Args>
void CScriptCallbackEx_<void>::operator()(Args&&... args) const
{
    try
    {
        try
        ...
        process_error catch (std::exception&) { ai().script_engine().print_output(ai().script_engine().lua(), "", LUA_ERRRUN); }
    }
    catch (...)
    {
        const_cast<CScriptCallbackEx_<return_type>*>(this)->clear();
...
template <>
template <typename... Args>
void CScriptCallbackEx_<void>::operator()(Args&&... args)
{
    try
    {
        try
