...
public:
	...
	typedef xr_vector< SEffect* > EffectVec; typedef EffectVec::iterator EffectVecIt;
	struct SSndChannel
	{
		...
		{
			CRandom rand;
			return sounds()[rand.randI(sounds().size())];
		}
		u32						get_rnd_sound_time		()
		{
			CRandom rand;
			return (m_sound_period.z < m_sound_period.w) ? rand.randI(m_sound_period.z,m_sound_period.w) : 0;
		}
		u32						get_rnd_sound_first_time()
		{
			CRandom rand;
			return (m_sound_period.x < m_sound_period.y) ? rand.randI(m_sound_period.x,m_sound_period.y) : 0;
		}
		float					get_rnd_sound_dist		()
		{
			CRandom rand;
			return (m_sound_dist.x < m_sound_dist.y) ? rand.randF(m_sound_dist.x, m_sound_dist.y) : 0;
		}
	...
	typedef xr_vector< SSndChannel* > SSndChannelVec; typedef SSndChannelVec::iterator SSndChannelVecIt;
protected:
	shared_str				m_load_section;
...
public:
	...
	{
		CRandom rand;
		return effects().empty()?0:effects()[rand.randI(effects().size())];
	}
	IC u32					get_rnd_effect_time ()
	{
		CRandom rand;
		return rand.randI(m_effect_period.x, m_effect_period.y);
	}
...
public:
	...
	typedef xr_vector< CEnvDescriptor* > EnvVec; typedef EnvVec::iterator EnvIt;
	typedef xr_map< shared_str,EnvVec, str_pred > EnvsMap; typedef EnvsMap::iterator EnvsMapIt;
private:
	// clouds
